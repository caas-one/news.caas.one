...
今年，我开始与Kubernetes合作，我的主要关注点之一是可伸缩性以及它如何影响您的应用程序和基础架构设计。当然，如果你添加自动缩放功能，那么这会是一个有趣的空间。
...
在Kubernetes中，有两个主要领域提供了缩放功能：

- 集群缩放-向集群添加和删除节点，以提供更多运行资源。这种缩放是在基础结构级别上完成的。
- 应用程序缩放-通过更改pods的特性来影响应用程序的运行方式。或者添加更多应用程序副本，或者更改可运行的资源。

但是，在您开始在应用程序级别进行缩放之前，我们首先需要确保集群提供了足够的资源来满足应用程序缩放的需要。让我们来看一下手动缩放我们的集群。 
...
主节点负责为集群提供一个控制平面，管理工作节点并确保集群内的一切顺利运行；则工作节点是我们需要缩放的。
...
大多数云提供程序允许您使用一个命令轻松地执行这些缩放操作。              下面是一个如何扩展Azure Kubernetes服务集群的示例：（docs）
...
为了做到这一点，我们的应用程序需要经过深思熟虑，并设计成可缩放的。
...
当您创建一个部署时，POD正在被部署和管理。这个规范将定义POD是什么，它需要什么特性，以及需要多少运行中的副本。在幕后，部署将创建一个副本集来管理部署的所有副本。              

但是，不建议创建和管理您自己的副本集，而是将其委托给部署，因为它具有更多的声明性方法。
...
这两个组件也有它们自己的缩放要求——我们的API必须根据CPU和内存进行缩放，而我们的订单队列工作人员需要根据队列上剩余的工作量进行缩放。              



这将指示实际更新副本集的部署更改请求的副本数。
...
您可以在[Kubernetes文档中](#creating-a-deployment)找到有关创建部署的更多信息。



**缩放pod**

创建部署后，手动缩放pods实例的数量实际上非常容易： 
...
就这样！部署将指示副本集更改所需数量的实例，这些实例将请求kubernetes提供更多实例，或者指示正在运行的pods正常关闭。             

 如果您再次获得部署的最新状态，您将注意到它已更改了所需的数量，并正在逐渐添加新的pods： 
...
我们已经了解到Kubernetes需要您扩展集群为应用程序提供足够的资源。应用程序还需要设计成可缩放的，我们使用了一个示例场景，向您展示了设计、部署和缩放它的潜在方法。             
