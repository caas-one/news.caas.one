...
由于日志记录代理必须在每个节点上运行，由于日志记录代理必须在每个节点上运行，因此通常将其实现为节点上的DaemonSet副本，manifest pod或专用本机进程。然而，后两种方法均被弃用且非常不建议使用。
...
注意： fluentd的配置超出了本文的范围。有关配置fluentd的信息，请参阅[官方流利文档](https://docs.fluentd.org/)
